...
**SQL Server**. El proyecto es un ejercicio académico de **Programación Orientada a Objetos (POO2)**
...
6. [Patrones de diseño y su importancia](#-patrones-de-diseño-y-su-importancia)
7. [Decisiones de diseño relevantes](#-decisiones-de-diseño-relevantes)
...
3. **Compilar:**
   ...
   mvn clean package
   ```
4. **Ejecutar la aplicación de escritorio** (punto de entrada: ventana de login):
   `com.laptitefrance.delivery.views.LoginView` → método `main`.
5. *(Opcional)* **Servidor REST** para repartidores:
   `com.laptitefrance.delivery.controllers.ApiRepartidor` → expone
   `http://localhost:8080/api/pedidos-pendientes`.
...
│   └── ApiRepartidor.java       Endpoint REST (Javalin) para la app del repartidor
│
├── views/             → Interfaz gráfica Swing (lo que ve el usuario)
│   ├── LoginView.java           Ventana de acceso (punto de entrada `main`)
...
│
├── events/            → Patrón Observer para cambios de estado de pedidos
│   ├── PedidoEstadoObservable.java   Sujeto observable
│   ├── ObservadorPedidoEstado.java   Interfaz observador
│   └── ConsolaRepartidorObservador.java  Observador concreto (demo)
│
...
│
└── audit/             → Trazabilidad
    └── AuditoriaLog.java        Registra acciones en auditoria_pedidos.txt (thread-safe)
```
